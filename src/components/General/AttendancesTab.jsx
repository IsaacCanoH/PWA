...
const AttendancesTab = ({ attendanceHistory, statistics, styles, themeClass }) => {
      ...
      <div className="row g-3 mb-4">
        <div className="col-12 col-md-6 col-lg-3 mx-auto">
          <div className={`card border-0 h-100 ${styles.bgPrimaryOpacity}`}>
            <div className="card-body text-center py-4">
              <BarChart3 size={32} className={`mb-3 ${styles.textPrimary}`} />
              <h4 className={`fw-bold mb-1 ${styles.textPrimary}`}>{statistics.porcentaje ?? 0}%</h4>
              <small className={`opacity-75 fw-medium ${styles.textPrimary}`}>Efectividad</small>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card border-0 shadow-sm">
        <div className="card-header bg-white border-0 py-4 d-flex justify-content-between align-items-center">
          <h6 className="mb-0 text-dark fw-semibold d-flex align-items-center">
            <Clock size={20} className={`me-2 ${styles.textPrimary}`} />
            Registros Recientes
          </h6>
              ...
              attendanceHistory.map((registro, index) => (
                <div key={index} className="list-group-item border-bottom border-0 py-4 px-4">
                  <div className="d-flex justify-content-between flex-wrap gap-3 align-items-center">
                    {/* Vista móvil - Layout responsivo */}
                    <div className="d-md-none w-100">
                      {/* Fecha y Estado - Responsivo */}
                    ...
                    {/* Vista desktop - Mantener exactamente igual */}
